...
3. **是哪一段链路？** 直接测上游那一跳 —— 从本机**直连住宅代理**只有 **1.26–1.77 MB/s**（4 并发 2.09），
   而隧道能做到 3.86，说明"边缘→住宅"远快于"客户端→住宅"。**瓶颈在上游住宅线路。**

结论：**这是一条链，速度由最慢的一段决定；换掉快的那一段没有意义。**
想再快只能换掉慢的那一段（换上游线路 / 加 VPS 入口 / 让大流量不走隧道），
而不是继续优化已经很快的那一段。**先量每一段，再决定动哪段。**
