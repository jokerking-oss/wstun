...
**"DNS 碰巧给的那个"在三轮独立测试里每次都是最慢的。** 换成优选 IP 后，TLS 握手
**390 ms → 131 ms**、吞吐 **≈4.3 → 6.4 MB/s**。

三个反直觉的坑，都写进 `docs/PITFALLS.md` 了：**延迟低 ≠ 带宽高**（延迟排名第一的 IP 吞吐只排
第四）；**并发测速会互相抢带宽把结果污染**（必须串行）；**连发太多会被 `speed.cloudflare.com`
限流返回 403**。还有一个隐藏前提：原 `connect_fast` 写的是 `if good in addrs`，要求记住的 IP 必须
出现在 DNS 结果里 —— 而优选出的 IP **通常不在**，于是优选永远轮不到。必须先修这一行。
...
docs/
  PITFALLS.md            # 踩坑记录：17 个真实的坑，含根因与解法
```
