...
**池子要开够大，否则"减少往返"就只对了那个命中池子的连接。** 第一版 `pool.up = 6` —— 恰好是浏览器
**每个域名**的并发上限，而一个页面常跨 2–4 个域名，于是**超出的连接全部退化成现场握手**。实测对比：

| 场景 | 建连耗时 |
|---|---|
| 池未命中（要现场 TCP+TLS+Upgrade） | **884 ms** |
| 池命中（直接取走预热好的通道） | **5 ms** |
| 调大池子后的端到端 TTFB | 1562 ms → **681 ms** |

把 `pool.up` 提到 `12`（`wstun.example.json` 的默认值）就吃掉了这个缺口。**预热只建到边缘的 WSS、
不占用任何住宅会话**，所以调大几乎零成本 —— 这也是为什么这个参数值得"宁可开大"。
...
> ⚠️ **这里有个必须说清的边界，而且我第一版归因推错了，值得当教材看。**
>
> 换完边缘 IP 后，**裸路径吞吐 4.3 → 6.4 MB/s，但隧道内只从 3.5 涨到 3.9**。于是我做归因：
> 1. **不是实现性能问题** —— 8 并发满速时，客户端进程 CPU 只占 **14% 单核**；
> 2. **不是通道数不够** —— 并发 1 / 4 / 8 的聚合是 **1.93 / 3.21 / 3.86 MB/s**，8 并发已近平台期；
> 3. **"是上游住宅代理那条腿"** —— 依据是"从本机直连住宅代理只有 1.26–1.77 MB/s"。
>
> **第 3 步是错的。** 从中国直连美国住宅测出的数字，**度量的是"中国↔美国链路"，不是住宅代理的能力** ——
> 中间任何一段烂链路都会冒充成被测段的上限。换成**从边缘节点（就在美国境内）去测那一跳**，结果是：
>
> ```
...
> 隧道已吃到其中约 **73%**。
>
> 所以正确的表述是：**优选边缘 IP 既降延迟、也对吞吐有实质贡献**（裸路径 +50%），
> 只是隧道聚合受跨国链路限制在 ≈3.9–4.4 MB/s。要再往上，只有：① 让吞吐型流量不走隧道
> （走边缘直出，代价是出口变成机房 IP）；② 加 VPS 入口；③ 换一条更快的跨国线路。
>
> **方法论**：测某一段的容量，**必须从离它最近的那一端发起**；每一步归因都要问
> "这个数字度量的是什么"，而不只是"这个数字是多少"。
...
> 真实天花板是**物理带宽**：隧道内聚合实测 ≈**3.9–4.4 MB/s（≈31–35 Mbps）**，限制来自
> **客户端 → 边缘的跨国链路**（不是上游住宅代理 —— 它从边缘侧测能跑 38 MB/s）。
> 1080p 流畅、4K 处于临界。详见上文性能一节的归因。
...
docs/
  PITFALLS.md            # 踩坑记录：21 个真实的坑，含根因与解法
```
...
`docs/PITFALLS.md` 里记了 21 条实证结论，例如：
